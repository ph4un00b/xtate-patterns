      ...
      on: {
        touchstart: {
          ...
          actions: [
            // setInitialPoint,
            assign({
              initialX: (_ctx, e) => e.touches[0].clientX,
              initialY: (_ctx, e) => e.touches[0].clientY,
            }),
          ],
        },
        mousedown: {
    ...
    dragging: {
      on: {
        touchmove: {
          ...
          actions: [
            assign({
              deltaX: (ctx, e: TouchEvent) =>
                e.touches[0].clientX - ctx.initialX,
              deltaY: (ctx, e: TouchEvent) =>
                e.touches[0].clientY - ctx.initialY,
            }),
          ],
        },
        mousemove: {
...
element?.addEventListener("touchstart", service.send);
body?.addEventListener("mousemove", service.send);
body?.addEventListener("touchmove", service.send);
body?.addEventListener("mouseup", service.send);
body?.addEventListener("touchend", service.send);
